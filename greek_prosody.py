...
def scan_marked_prosody (line):
    """Scans a string of Greek poetry or prose, as best as possible without 
    knowing whether ambiguous vowels are long by nature. Returns the meter as a
    list of syllables marked as follows:
        'L-NAT' -   Long by nature
        'S-POS-L'  -   Short by nature, Long by position
        'X-POS-L'  -   Unknown natural quantity, Long by position 
        'S-POS-S'  -   Short by nature and by position
        'X-POS-S'  -   Unknown natural quantity, Short by position
        'S-POS-X'  -   Short by nature, Uncertain positional length
        'X-POS-X'  -   Unknownnatural quantity, Uncertain positional length
    
    If resolutions have been marked in the text with '|', the resulting 
    bi-syllable is marked RESOLUTION ('R'). These are not checked for positional
    length, accepting the marked resolution as authoritative.
    """
    line = line.strip()
    meter = []
    syl_list = get_syllables(line, resolutions=True)
    
    #Iterate through syllables checking prosody
    for i, syl in enumerate(syl_list):
        #Check for Resolution
        if '|' in syl:
            meter.append('R')
            continue
        nat_len = natural_length(syl)
        
        #Long by Nature
        if nat_len == 'L':
            length = 'L-NATURE'
        
        #'S' or 'X' by Nature: Check Positional Length
        else:
            if i == len(syl_list)-1: 
                next_syl = 'END'
            else:
            ...
            #Long by Position   
            if pos_len == 'L':
                length = nat_len + '-POS-L'
            
            #Short by Position
            elif pos_len == 'S':
                length = nat_len + '-POS-S'
            
            #Indeterminate Position
            elif pos_len == 'X':
                length = nat_len + '-POS-X'
        meter.append(length)

    return meter

def scan_resolved (bisyl, next_syl):
    """Splits a syllable where a resolution has been marked with '|' into its 
    constituent parts and checks that both syllables are short. 
    Returns RESOLVED ('R') if everything looks good.
    """
    syl_a, syl_b = bisyl.split('|')
    error_message = 'Problem with resolved syllable {}'.format(bisyl)
    assert natural_length(syl_a) != LONG, error_message
    assert natural_length(syl_b) != LONG, error_message
    assert positional_length(syl_a, syl_b) != LONG, error_message
    assert positional_length(syl_b, next_syl) != LONG, error_message
    return RESOLVED
#%%
